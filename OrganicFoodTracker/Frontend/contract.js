...
App = {
  ...
  loadRetailerContract: async() => {
	...
	},
	{
		...
		"name": "getIncomingProcessorAddress",
		"outputs": [
			{
				"name": "",
				"type": "address"
 ...
 getIncomingProductsRetailer: async (data) => {
   ...
   for (var i = 0; i < incoming_products.length; i++) {
     ...
     if (incoming_id > 0) {
       //get current product info
       const incoming_product_id = await App.retailerContract.methods.getIncomingProductId(incoming_id).call();
       const incoming_weight = await App.retailerContract.methods.getIncomingWeight(incoming_id).call();
       const time_sent = await App.retailerContract.methods.getIncomingTime(incoming_id).call();
       var date = new Date(time_sent*1000);
       const processor_address = await App.retailerContract.methods.getIncomingProcessorAddress(incoming_id).call();
       //product values to be output to the screen
       let output =
             `<a href="#" class="list-group-item list-group-item-action" data-toggle="list" role="tab">
                 <div class="d-flex w-100 justify-content-between">
                   ...
                   <p class="mb-1">Weight: `+ incoming_weight +`</p>
                   <p class="mb-1">Time/Date Sent: `+ date +`</p>
                   <p class="mb-1">Farmer Address: `+ processor_address +`g</p>
               </a>`;
 ...
 recieveProductRetailer: async (data) => {
   ...
   if ((incoming_id == ""|| incoming_weight == "") || isNaN(incoming_weight_int) || isNaN(incoming_id_int)){
     ...
     let sender = await App.retailerContract.methods.getIncomingProcessorAddress(incoming_id_int).call(); //get sender address
